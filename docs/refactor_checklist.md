...
- [ ] `draco_tools/core/encoder.py`를 단일 진입점으로 사용하도록 `draco_roundtrip.nodes.stream_client`와 `draco_tools.cli.encode_ply_to_draco`가 동일 함수 시그니처를 호출하도록 리팩토링한다. → 중복 옵션 파싱 제거, 통합된 로그 포맷 문서화.
