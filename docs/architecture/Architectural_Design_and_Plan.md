# 아키텍처 설계 및 지연 시간 계획
이 문서는 Draco Roundtrip의 비동기 스트리밍 파이프라인 구조, 최신 프로토콜(v2) 통합, 그리고 네트워크 지연 시간 단축 로드맵을 정리합니다.
...
[Capture Thread] --bounded--> [Encode Worker Pool] --bounded--> [TX Loop]
      ...
      │<--backpressure------------------------------------------│
      ▼                                                         ▼
[Network Transport] <== 단일 TCP 연결(FrameType) ==> [RX Pump] --bounded--> [Decode Pool] --bounded--> [Publish Thread]
```
...
- **전송 단계**: TX 루프는 논블로킹 소켓을 사용하며 시퀀스별 전송 시간을 기록하고 `max_inflight` 윈도를 준수합니다. 페이로드는 통합 프레임 헤더(`magic=b"DRC0"`, `version=2`, 타임스탬프/콘텐츠 타입 내장)와 선택적 분할(`--tx-fragment-size`)을 사용합니다.
- **RX 펌프 및 디코드 단계**: 전용 리더가 단일 소켓에서 DATA/ACK/HEARTBEAT/EOF/ERROR 프레임을 순차 처리하고 ACK 시 in-flight 테이블을 갱신하며, 디코드된 페이로드를 제한된 디코드 큐로 전달합니다. `next_seq` 기반 재정렬 버퍼가 퍼블리시 순서를 보존합니다.
...
## 제어 플레인과 제한 큐
- **메시지 코드**(`FrameType.DATA/ACK/HEARTBEAT/EOF/ERROR`)는 [프로토콜 및 스키마 참조](../reference/Protocol_and_Schema_Reference.md)에 정의된 단일 프레임 헤더를 공유합니다. `FrameType`은 플래그 하위 4비트에 저장되어 **별도 제어 소켓 없이** 멀티플렉싱을 수행합니다.
- **상태 기계**: `INIT → HANDSHAKING → STREAMING → DRAINING → TERMINATED` 흐름을 따르며, 실패 시 `FAILED`로 이탈합니다. EOF 전이는 `pending=0` 조건이 충족되어야 완료됩니다.
- **하트비트 정책**: 2 초마다 하트비트를 전송하고 6 초간 응답이 없으면 타임아웃으로 간주합니다. 누락된 하트비트는 `TIMEOUT` 코드의 `ERROR`를 발생시켜 큐를 비웁니다.
...
- **프래그먼테이션**: `--tx-fragment-size`가 0보다 크면 TX 루프가 `FLAG_FRAGMENTED`/`FLAG_MORE_FRAGMENTS` 플래그를 포함한 MTU 안전 조각(256–1400 B)을 전송하며, 수신 측은 재조립 후 ACK를 발송합니다.
- **프래그먼트 GC**: 서버의 `FragmentBuffer`는 마지막 업데이트 타임스탬프를 추적하며 60 초마다 유휴 항목을 삭제합니다. 총 누적 바이트가 `FRAGMENT_BUFFER_MAX_BYTES`를 초과하면 가장 오래된 항목부터 제거하고 경고를 남깁니다.
...
3. **바이너리 프로토콜**: v2 헤더는 타임스탬프/콘텐츠 타입을 포함한 26바이트 고정 길이를 사용합니다. `--legacy-mode`가 활성화되지 않으면 v1(`b"DRTC"`) 프레임은 거절됩니다.
...
6. **벤치마크**: [성능 시험 계획](../development/Performance_Test_Plan.md)에 설명된 rosbag 기반 벤치마크와 텍스트 프로토콜 상한 테스트로 개선 효과를 검증합니다.
...
## 시퀀스 다이어그램
다음 다이어그램은 제한 큐, ACK 기반 흐름 제어, 단일 소켓 멀티플렉싱을 포함한 최신 하이브리드 파이프라인을 설명합니다.
