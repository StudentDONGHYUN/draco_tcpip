...
2. `/home/kkit/newdisk/draco_tcpip/configs/rtabmap_stream.yaml`
   ...
   - **분석**:
     - SLAM 노드 설정:
       - 프레임 설정
         - `frame_id`와 `odom_frame_id`가 LiDAR 데이터가 게시되는 `lidar_frame`/`odom` 트리와 일치하도록 구성됨. 이전 `base_link` 설정은 스트리밍 데이터에 대한 TF 변환이 실패해 포인트 클라우드 처리가 지연될 수 있었음. 입력 프레임을 정렬하면 TF 브로드캐스터가 필요 없는 직접 매칭이 가능해지고 프레임 누락으로 인한 오도메트리 드롭을 방지함.
     - 메모리 관리
     - 루프 클로저 파라미터
     - 그리드 맵 설정
     - ICP 오도메트리 노드 설정:
