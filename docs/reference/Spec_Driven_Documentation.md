...
- **품질 기준**: 모든 문서/실행은 [소프트웨어 품질 특성 카탈로그](../reference/Quality_Attributes_Catalog.md)를 기준으로 기능성·성능·신뢰성·보안성·운영성·사용성·테스트 가능성을 점검합니다.
...
4. **검증·증거** — 사양을 만족함을 입증하는 테스트와 보고.
   ...
   - [결과 템플릿](../quality/results_template.md) & [리포트/결과 템플릿](../reports/results_template.md): 실행 보고서 표준 및 품질 속성 체크리스트.
...
4. **품질 특성 검증**: 변경이 어떤 품질 특성(성능, 보안, 운영성 등)에 영향을 주는지 [품질 카탈로그](../reference/Quality_Attributes_Catalog.md) 표에 연결하고, 영향을 받는 템플릿/테스트를 갱신합니다.
5. **운영 가이드 정합성**: 사용 흐름이나 로그 수집 방식이 달라지면 [사용자 가이드](../guides/User_Guide.md)와 런타임 메모를 업데이트합니다.
6. **근거 수집**: 실행 결과를 [결과 템플릿](../quality/results_template.md)으로 기록하고, 회귀 위험은 [리팩터 및 감사 로그](../reports/Refactor_and_Audit_Log.md)에 남깁니다.
