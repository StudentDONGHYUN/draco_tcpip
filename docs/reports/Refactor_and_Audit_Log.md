...
## 요약
- 단일 TCP 연결에서 FrameType 기반 멀티플렉싱을 사용하고, 통합 헤더(v2)가 타임스탬프/콘텐츠 타입을 포함하도록 업데이트되었습니다.
- 프래그먼트 GC와 텍스트 프로토콜 크기 제한이 서버/클라이언트에 적용되어 장시간 세션과 악의적 입력에 대비합니다.
- 문서·CLI·자동화 아티팩트가 프로파일, QoS, 인코더 옵션뿐 아니라 프로토콜 버전 및 레거시 모드에 대한 공용 참조를 사용하도록 정리되었습니다.
...
| 7 | 단일 소켓 멀티플렉싱, 프래그먼트 GC, 텍스트 제한을 문서/테스트/코드에 반영했습니다. |
...
- [ ] 텍스트 프로토콜 제한 및 프래그먼트 GC 경고가 운영 대시보드에 노출되도록 텔레메트리 파이프라인을 확장합니다.
