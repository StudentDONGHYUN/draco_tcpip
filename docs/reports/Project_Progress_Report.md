...
## 요약
비동기 파이프라인과 단일 TCP 기반 제어 플레인 목표는 대부분 안정화되었습니다. 통합 헤더(v2), 프래그먼트 GC, 텍스트 프로토콜 제한이 운영 단계에 있지만 회귀 벤치마킹, 고급 전송, 인프로세스 Draco는 아직 남아 있습니다. 장기 과제인 QUIC/UDP-FEC, C++ 포트, 텔레메트리 기반 적응 제어는 설계 검토 단계입니다.

## 단기 계획
- rosbag 회귀 벤치마크 하네스를 구축하고 p95 지연 기준을 자동으로 확인합니다.
- 네이티브 이벤트 API가 없는 플랫폼에서 파일 시스템 워처 성능을 측정하고 개선 전략을 문서화합니다.
- 단일 소켓 멀티플렉싱과 프래그먼트 GC를 포함한 엔드투엔드 테스트와 로깅 범위를 확장합니다.
